...
const resetGame = async () => {
  try {
    console.log("重置游戏状态，准备新一轮游戏...");
    // 这里不需要删除任何记录，因为我们只是标记新的一天可以进行新一轮的投注
    // 所有的约束条件都基于日期，新的一天会自动开始新的游戏
    return { success: true, message: "游戏已重置，可以开始新一轮" };
  } catch (error) {
    console.error("重置游戏失败:", error);
    return { success: false, message: "重置游戏失败", error: error.message };
  }
};
...
exports.getTodayBet = async (req, res) => {
  try {
    ...
    console.log("查询用户今日投注:", { userId, todayDate });
    ...
    console.log("用户今日投注查询结果:", rows);

    if (rows.length === 0) {
      console.log("今日没有找到投注记录");
      return res.status(404).json({ message: "今日没有找到投注记录" });
    }
...
exports.getDrawResult = async (req, res) => {
  try {
    ...
    console.log("查询今日开奖结果:", { userId, todayDate });
    ...
    console.log("今日开奖结果查询:", resultRows);
    ...
    if (resultRows.length === 0) {
      console.log("今日没有找到开奖结果");
      return res.status(404).json({ message: "今日没有找到开奖结果" });
    }

    const result = resultRows[0];

    // 获取用户今日的投注信息
    console.log("查询用户今日投注信息...");
