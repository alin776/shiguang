...
const getUserPoints = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "获取积分信息失败" });
  }
};
...
const getPointsHistory = async (req, res) => {
  try {
    ...
    console.log("获取用户积分历史记录", { userId, page, pageSize });

    // 获取总记录数
    const [countResult] = await db.execute(
      `SELECT COUNT(*) as total FROM user_points_history WHERE user_id = ?`,
      [userId]
    );
    ...
    res.status(500).json({ message: "获取积分历史记录失败" });
  }
};
