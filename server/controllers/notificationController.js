  ...
  try {
    ...
    const [notifications] = await db.execute(
      `SELECT 
        ...
        CASE
          -- 帖子类型直接使用source_id作为post_id
          WHEN n.source_type = 'post' THEN n.source_id
          -- 评论类型的通知，source_id已经是post_id
          WHEN n.source_type = 'comment' THEN n.source_id
          -- 回复类型通知，直接使用source_id，因为在创建通知时已存储了帖子ID
          WHEN n.type = 'reply' THEN n.source_id
          -- 点赞评论的通知，需要从评论表中查询对应的帖子ID
          WHEN n.type = 'like' AND n.source_type = 'comment' THEN (
            SELECT c.post_id FROM comments c WHERE c.id = n.source_id
          )
          -- 关注类型通知，没有关联帖子，设为NULL
      ...
      notifications: notifications.map((notification) => {
        ...
        return {
          ...
          actor: notification.actor_id
            ...
            : null,
          post_id: notification.post_id // 确保返回帖子ID供前端使用
        };
      }),
