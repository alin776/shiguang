<template>
  <div class="posts-page">
    ...
    <div class="posts-list card">
      ...
      <!-- 无数据或无分页时显示提示 -->
      <el-empty v-if="!loading && (!posts || posts.length === 0)" description="暂无数据" />
    </div>
  </div>
</template>
...
// 初始化
onMounted(async () => {
  console.log('开始初始化帖子管理页面')
  await fetchPosts()
  await fetchCategories()
  console.log('初始化完成，分页数据:', pagination.value)
})
...
const fetchPosts = async () => {
  try {
    console.log('开始获取帖子列表，当前分页:', pagination.value)
    await postStore.fetchPosts()
    console.log('获取帖子列表完成，更新后分页:', pagination.value)
  } catch (error) {
