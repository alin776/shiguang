...
export default {
  setup() {
    // 数据和状态
    const API_BASE_URL = 'http://47.98.210.7:3000'; // 直接硬编码API地址
    console.log('设置的API地址:', API_BASE_URL);
    ...
    const loadLatestVersions = async () => {
      ...
      try {
        ...
        console.log('最新版本响应:', response.data);
        
        if (response.data.success) {
          latestVersions.value = response.data.versions;
        } else {
          console.error('加载版本信息返回失败状态:', response.data);
          ElMessage.error(response.data.message || '加载版本信息失败');
        }
      } catch (error) {
        console.error('获取最新版本失败详情:', error);
    ...
    const loadVersionHistory = async () => {
      ...
      try {
        console.log('正在加载版本历史，平台:', selectedPlatform.value);
        console.log('API BASE URL:', API_BASE_URL);
        ...
        console.log('版本历史响应:', response.data);
        
        if (response.data.success) {
          versionHistory.value = response.data.versions;
        } else {
          console.error('加载版本历史返回失败状态:', response.data);
          ElMessage.error(response.data.message || '加载版本历史失败');
        }
      } catch (error) {
        console.error('获取版本历史失败详情:', error);
        console.error('错误响应详情:', error.response?.data);
